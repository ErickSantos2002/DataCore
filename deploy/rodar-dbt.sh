...
# De onde veio o disparo — mesma regra do `rodar-job.sh`: `INVOCATION_ID` so existe
# quando quem executa e o systemd. Vai para dentro do container porque quem grava o
# registro do dbt e a macro `registrar_execucao_dbt`, que le esta variavel.
#
...
if [ $# -gt 0 ]; then
    ...
    exec docker exec -e DATACORE_ORIGEM_JOB="$ORIGEM_JOB" "$CONTAINER" dbt "$@"
fi
...
while :; do
    ...
    codigo=0
    docker exec -e DATACORE_ORIGEM_JOB="$ORIGEM_JOB" "$CONTAINER" dbt build || codigo=$?
