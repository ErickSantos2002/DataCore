...
class ExecucaoJob(Base):
    ...
    # `agendada` (o systemd disparou) ou `manual`. Ver `_origem()` e a migration 010.
    origem = Column(Text, nullable=False, server_default=text("'manual'"))


def _origem() -> str:
    """De onde veio esta execução, para a estatística de duração não misturar as duas.

    Quem sabe a resposta é o DISPARADOR, não o job: o mesmo `python -m app.jobs.x` roda
    pelo timer às 04:00 e pela mão do Erick às 15:00. Por isso `rodar-job.sh` e
    `rodar-dbt.sh` exportam `DATACORE_ORIGEM_JOB` para dentro do container — eles sabem
    se quem os chamou foi o systemd (a variável `INVOCATION_ID` só existe lá).

    O padrão é `manual` porque é o valor seguro: uma execução manual classificada como
    agendada estraga a mediana; uma agendada classificada como manual só a deixa com
    menos base, o que a tela mostra.
    """
    valor = (os.environ.get("DATACORE_ORIGEM_JOB") or "").strip().lower()
    return valor if valor in ("agendada", "manual") else "manual"
...
def registrar_execucao(job: str, argumentos: str = "") -> Iterator[Registro]:
    ...
    try:
        ...
        linha = ExecucaoJob(job=job, inicio=datetime.now(timezone.utc),
                            argumentos=argumentos or None, origem=_origem())
        sessao.add(linha)
        sessao.commit()
        linha_id = linha.id
