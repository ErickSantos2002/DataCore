...
- `/auth/login` → Autenticação JWT (obtém `access_token`)
- `/auth/me` → Dados do usuário autenticado
- `/auth/users` → Gestão de usuários (admin)
...
```bash
git clone git@github.com:ErickSantos2002/DataCore.git
cd DataCore/backend
```
...
- Autenticação **JWT via `/auth/login`** já implementada; defina `SECRET_KEY` forte e diferente da do authapi (gere com `python -c "import secrets; print(secrets.token_urlsafe(48))"`)
- Mude `AUTH_OBRIGATORIA=true` após as logs não mostrarem mais "Acesso sem token"
...
- [x] Implementar autenticação com token seguro
